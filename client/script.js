...
sayHelloButton.addEventListener('mouseout', helloDefault)
...
const ohMy = (event) => {
    ...
    .then((response) => {
        ...
        for (let i = 0; i < response.data.length; i++) {
            document.createElement('p').setAttribute('id', `p${i}`)
            document.getElementById(`p${i}`).textContent = response.data[i]
        }
    })
    .catch((err) => {
        console.log(err + ': error on ohMy')
    })
}
...
const repeatMyParam = (event) => {
    axios.get(`http://localhost:3000/repeat/auston`)
    .then((response) => {
        console.log(response.data)
    ...
    .catch((err) => {
        console.log(err + ' repeatMyParam error')
    })
}
...
function qR (event) {
    event.preventDefault()
    axios.get('http://localhost:3000/query-test?does-work=true')
    .then((response) => {
        console.log(response)
    })
    .catch ((err) => {
        console.log(err + ' qR failure')
    })
}

document.getElementById('query-button').addEventListener('click', qR)
